#!/usr/bin/python3

print("This is my first script for this file and has been modified to show how to handle a change in my code")
for _ in range(10):
    print("Hello world!")

while True:
    print("Hello world for ever!")
    break

print("The above is an infinite loop and this print will never happen")
print("This is another line added to the master branch")
print("This is another line added to the side branch")
print("This is another line added to the side branch which I will send to the remote server")
print("This is another line initially added to the new_branch. Going again and again leads to knowledge22/3/23")
